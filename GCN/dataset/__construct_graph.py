...
def Knn_graph(data,edge_type,label,edge_norm,K):
    ...
    y = torch.tensor(label * np.ones(data.shape[0]), dtype=torch.long)  # 节点标签
    # adj = torch_geometric.utils.to_scipy_sparse_matrix(edge_index, edge_attr)
    ...
    # graph = Data(x=x, edge_index=edge_index, y=y, edge_attr=edge_attr, svd_v=svd_v, svd_u=svd_u, u_mul_s=u_mul_s)  # 图
